...
struct mtk_disp_rdma_data {
	...
	const u32 *formats;
	size_t num_formats;
	unsigned int mem_start_addr_reg;  /* 0 -> default DISP_RDMA_MEM_START_ADDR (0x0f00); MT6582 (layout antiguo) = 0x28 */
};
			   ...
			   struct cmdq_pkt *cmdq_pkt)
{
	...
	mtk_ddp_write_relaxed(cmdq_pkt, addr, &rdma->cmdq_reg, rdma->regs,
			      rdma->data->mem_start_addr_reg ?: DISP_RDMA_MEM_START_ADDR);
...
/* MT6582 (BQ krillin): mismo RDMA que mt2701 (fifo SZ_4K) PERO el registro
 * MEM_START_ADDR esta en el layout ANTIGUO @0x28 (no 0x0f00). */
static const struct mtk_disp_rdma_data mt6582_rdma_driver_data = {
	.fifo_size = SZ_4K,
	.formats = mt8173_formats,
	.num_formats = ARRAY_SIZE(mt8173_formats),
	.mem_start_addr_reg = 0x28,
};
