...
/* MT6582 (BQ Aquaris E4.5 "krillin"): misma generacion que mt2701. Sin CCF ->
 * clk_driver = NULL (el LK deja el DISPSYS encendido + clk_ignore_unused).
 * Routing reusa el default de mt2701 en Fase 1 (no se ejecuta sin componentes
 * conectados); el OVL_MOUT_EN propio del MT6582 (0x030) se ajusta en Fase 2.
 * Sin reset controller (num_resets = 0; los bloques los enciende el LK). */
...
static int mtk_mmsys_probe(struct platform_device *pdev)
{
	...
	/* MT6582: clk_driver = NULL (sin CCF) -> no registrar el clk pdev del MM,
	 * el LK deja el DISPSYS encendido. (Bloqueante F del roadmap.) */
	if (mmsys->data->clk_driver) {
...
static const struct of_device_id of_match_mtk_mmsys[] = {
	{ .compatible = "mediatek,mt6582-mmsys", .data = &mt6582_mmsys_driver_data },
