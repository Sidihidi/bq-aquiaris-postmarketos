...
/* MT6323 PMIC ANALDO_CON17 (0x418): bit12=RG_VCN33_EN_WIFI (lo pone el regulator),
 * bit14=RG_VCN33_ON_CTRL_WIFI = "modo HW" (el HW del MAC WiFi controla el rail RF).
 * El OEM lo pone a 1 en func_on(WIFI) (ahb.c:1993 upmu_set_vcn33_on_ctrl_wifi(1)); sin él
 * el MAC no arranca y WCIR.WLAN_READY nunca se afirma. El regmap del pwrap direcciona MT6323. */
#define MT6323_ANALDO_CON17		0x0418
#define VCN33_ON_CTRL_WIFI_HW		(1U << 14)
/* MT6323 ANALDO_CON16 (0x416): bit5=RG_VCN33_ON_CTRL_BT = "modo HW" (el HW del BT/combo
 * controla el rail RF). El stock lo pone a 1 (upmu_set_vcn33_on_ctrl_bt(1)) en la RF-cal
 * (junto con el de WiFi). El ROMv1 lo NECESITA para que la RF-calibration del patch no
 * cuelgue el chip (RF-CAL -110). */
#define MT6323_ANALDO_CON16		0x0416
#define VCN33_ON_CTRL_BT_HW		(1U << 5)
...
struct mt6582_consys {
	...
	dma_addr_t emi_phys;
	struct regmap *pmic_regmap;	/* regmap pwrap->MT6323 (modo-HW de VCN33_WIFI) */
};
...
/* puntero global para que el driver WiFi controle VCN33_WIFI en su func_on (como HifAhbProbe). */
static struct mt6582_consys *g_consys;

/*
 * Enciende/apaga VCN33_WIFI (LDO RF del WiFi). El OEM lo enciende en HifAhbProbe (= func_on(WIFI)),
 * NO al arrancar el CONSYS: el MAC WiFi necesita el flanco off->on coincidente con el boot de su
 * firmware o nunca afirma WLAN_READY. BT/GPS NO usan este rail. Exportada para mt6582-wifi.c.
 */
int mt6582_consys_wifi_vcn33(bool on)
{
	struct mt6582_consys *cs = g_consys;
	int ret;

	if (!cs || IS_ERR_OR_NULL(cs->vcn33))
		return -ENODEV;

	if (on) {
		/* 1) hwPowerOn: enable del LDO (ANALDO_CON17 bit12, vía el regulator) */
		ret = regulator_enable(cs->vcn33);
		/* 2) upmu_set_vcn33_on_ctrl_wifi(1): conmutar a MODO HW (bit14) — EL PASO QUE FALTABA.
		 * Sin esto el MAC WiFi no secuencia su power RF y no afirma WLAN_READY. */
		if (!IS_ERR_OR_NULL(cs->pmic_regmap))
			regmap_update_bits(cs->pmic_regmap, MT6323_ANALDO_CON17,
					   VCN33_ON_CTRL_WIFI_HW, VCN33_ON_CTRL_WIFI_HW);
		return ret;
	}
	/* apagar: volver a modo SW (bit14=0) y luego disable */
	if (!IS_ERR_OR_NULL(cs->pmic_regmap))
		regmap_update_bits(cs->pmic_regmap, MT6323_ANALDO_CON17,
				   VCN33_ON_CTRL_WIFI_HW, 0);
	return regulator_disable(cs->vcn33);
}
EXPORT_SYMBOL_GPL(mt6582_consys_wifi_vcn33);

/* PALDO BT (VCN33 modo-HW por el path del BT, bit5 de 0x416). El stock lo enciende junto
 * con el de WiFi alrededor de la RF-calibration del CONSYS (imprescindible para el ROMv1). */
int mt6582_consys_bt_vcn33(bool on)
{
	struct mt6582_consys *cs = g_consys;
	int ret;
	...
	if (on) {
		ret = regulator_enable(cs->vcn33);
		if (!IS_ERR_OR_NULL(cs->pmic_regmap))
			regmap_update_bits(cs->pmic_regmap, MT6323_ANALDO_CON16,
					   VCN33_ON_CTRL_BT_HW, VCN33_ON_CTRL_BT_HW);
		return ret;
	}
	if (!IS_ERR_OR_NULL(cs->pmic_regmap))
		regmap_update_bits(cs->pmic_regmap, MT6323_ANALDO_CON16,
				   VCN33_ON_CTRL_BT_HW, 0);
	return regulator_disable(cs->vcn33);
}
EXPORT_SYMBOL_GPL(mt6582_consys_bt_vcn33);

static void consys_activate_mcu(struct mt6582_consys *cs);

/*
 * HW-reset del CONSYS = réplica del opfunc_hw_rst del stock (capturado en LineageOS 0716:
 * el stock apaga y reenciende el MTCMOS+rails JUSTO antes de descargar el patch — la
 * descarga es el PRIMER tráfico WMT que ve el ROM tras el reset). Apagado en orden
 * inverso al probe; reencendido idéntico. Exportado para el bring-up del btif.
 */
int mt6582_consys_hw_rst(void)
{
	struct mt6582_consys *cs = g_consys;
	void __iomem *spm, *infra;
	int t, ret;
	u32 id = 0;
	...
	dev_info(cs->dev, "HW-RST del CONSYS (MTCMOS+rails off->on, como el stock)\n");
	mt6582_consys_ready = false;

	/* apagado: proteger bus, SRAM down, aislar, reset, clock-dis, power off */
	writel(SPM_KEY, spm + SPM_POWERON_CFG);
	writel(readl(infra + TOPAXI_PROT_EN) | CONN_PROT_MASK, infra + TOPAXI_PROT_EN);
	for (t = 0; t < 2000; t++) {
		if ((readl(infra + TOPAXI_PROT_STA1) & CONN_PROT_MASK) == CONN_PROT_MASK)
			break;
		udelay(10);
	}
	writel(readl(spm + SPM_CONN_PWR_CON) | CONN_SRAM_PDN, spm + SPM_CONN_PWR_CON);
	...
	writel(readl(spm + SPM_CONN_PWR_CON) & ~PWR_RST_B, spm + SPM_CONN_PWR_CON);
	writel(readl(spm + SPM_CONN_PWR_CON) | PWR_CLK_DIS, spm + SPM_CONN_PWR_CON);
	writel(readl(spm + SPM_CONN_PWR_CON) & ~(PWR_ON | PWR_ON_S), spm + SPM_CONN_PWR_CON);
	for (t = 0; t < 2000; t++) {
		if (!(readl(spm + SPM_PWR_STATUS) & CONN_PWR_STA_MASK) &&
		    !(readl(spm + SPM_PWR_STATUS_S) & CONN_PWR_STA_MASK))
			break;
	...
	/* rails analógicos off (el stock también los cicla en su hw_rst) */
	if (!IS_ERR_OR_NULL(cs->vcn28))
		regulator_disable(cs->vcn28);
	if (!IS_ERR_OR_NULL(cs->vcn18))
		regulator_disable(cs->vcn18);
	msleep(10);

	/* reencendido: orden del probe (rails -> MTCMOS -> MCU) */
	if (!IS_ERR_OR_NULL(cs->vcn18))
		regulator_enable(cs->vcn18);
	if (!IS_ERR_OR_NULL(cs->vcn28))
		regulator_enable(cs->vcn28);
	...
	if (id != CONSYS_CHIP_ID_6582 && id != CONSYS_CHIP_ID_6572) {
		dev_err(cs->dev, "HW-RST: CONSYS no responde (chip-id=0x%08x)\n", id);
		return -ENODEV;
	}
	dev_info(cs->dev, "HW-RST OK: chip-id=0x%04x, ROM corriendo\n", id);
	mt6582_consys_ready = true;
	return 0;
}
EXPORT_SYMBOL_GPL(mt6582_consys_hw_rst);

/* Lectura del PC del MCU nds32 (CPUPCR, CONN_MCU_CONFIG+0x160) — diagnóstico del
 * salto al fw parcheado tras el WMT_RESET (¿dónde arranca y dónde descarrila?). */
u32 mt6582_consys_cpupcr(void)
{
	struct mt6582_consys *cs = g_consys;

	if (!cs || !cs->mcu_cfg)
		return 0;
	return readl(cs->mcu_cfg + 0x160);
}
EXPORT_SYMBOL_GPL(mt6582_consys_cpupcr);
...
static int mt6582_consys_probe(struct platform_device *pdev)
{
	...
	/* regmap del pwrap (direcciona los regs del MT6323) para el modo-HW de VCN33_WIFI */
	{
		struct device_node *pw = of_find_compatible_node(NULL, NULL,
								 "mediatek,mt6582-pwrap");
		struct platform_device *pwdev = pw ? of_find_device_by_node(pw) : NULL;

		of_node_put(pw);
		cs->pmic_regmap = pwdev ? dev_get_regmap(&pwdev->dev, NULL) : NULL;
		if (!cs->pmic_regmap)
			dev_warn(dev, "pwrap regmap no disponible (VCN33 modo-HW se omitirá)\n");
	}
	...
	if (ret)
		goto err_v18;
	/*
	 * VCN33_WIFI NO se enciende aquí: lo hace el driver WiFi en func_on(WIFI)
	 * (mt6582_consys_wifi_vcn33), imitando HifAhbProbe del OEM — el MAC necesita
	 * el flanco off->on coincidente con el boot del firmware. BT/GPS no usan este rail.
	 */
	...
	if (id == CONSYS_CHIP_ID_6582 || id == CONSYS_CHIP_ID_6572) {
		...
		g_consys = cs;			/* habilita mt6582_consys_wifi_vcn33() para el WiFi */
...
static void mt6582_consys_remove(struct platform_device *pdev)
{
	struct mt6582_consys *cs = platform_get_drvdata(pdev);
	...
	g_consys = NULL;
	if (cs->emi_virt)
		dma_free_coherent(cs->dev, SZ_1M, cs->emi_virt, cs->emi_phys);
	regulator_disable(cs->vcn33);
	regulator_disable(cs->vcn28);
	regulator_disable(cs->vcn18);
	if (cs->spm)     iounmap(cs->spm);
	if (cs->infra)   iounmap(cs->infra);
	if (cs->mcu_cfg) iounmap(cs->mcu_cfg);
}
